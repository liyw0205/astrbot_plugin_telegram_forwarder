...
## 模块职责
`core/senders` 是本插件负责实际将消息分发到不同目标平台（主要为 QQ 和 Telegram）的执行层。尤其针对 QQ 发送，由于目标平台协议复杂、网络不稳定、载荷体积及特殊媒体支持差异大，因此该子模块对发送链条进行了极其细致的门面模式解耦：`QQSender` 只负责编排，所有可独立的细节下沉到 13 个职责单一的子模块，便于测试与演进。
